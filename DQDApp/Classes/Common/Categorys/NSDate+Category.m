...
- (NSString *)defaultFamattedTimeStr{
    
    NSDateFormatter *dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"yyyy年MM月dd日 HH时mm分"];
...
- (NSString *)minuteDescription
{
    ...
    if ([theDay isEqualToString:currentDay]) {//当天
        ...
        [dateFormatter setDateFormat:@"ah:mm"];
        return [NSString stringWithFormat:NSLocalizedString(@"昨天 %@", @'"'), [dateFormatter stringFromDate:self]];
...
-(NSString *)formattedTime{
    ...
    if (!hasAMPM) { //24小时制
        if (hour <= 24 && hour >= 0) {
            ...
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"昨天HH:mm", @"")];
        }else {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"yyyy-MM-dd HH:mm"];
        }
    }else {
        if (hour >= 0 && hour <= 6) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"凌晨hh:mm", @"")];
        }else if (hour > 6 && hour <=11 ) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"上午hh:mm", @"")];
        }else if (hour > 11 && hour <= 17) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"下午hh:mm", @"")];
        }else if (hour > 17 && hour <= 24) {
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"晚上hh:mm", @"")];
        }else if (hour < 0 && hour >= -24){
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:NSLocalizedString(@"昨天HH:mm", @"")];
        }else  {
...
- (NSString *)formattedDateDescription
{
    ...
    if (timeInterval < 60) {
        ...
        return [NSString stringWithFormat:NSLocalizedString(@"今天 %@", @""), [dateFormatter stringFromDate:self]];
	} else if ([[dateFormatter dateFromString:currentDay] timeIntervalSinceDate:[dateFormatter dateFromString:theDay]] == 86400) {//昨天
        [dateFormatter setDateFormat:@"HH:mm"];
        return [NSString stringWithFormat:NSLocalizedString(@"昨天 %@", @""), [dateFormatter stringFromDate:self]];
    } else {//以前
